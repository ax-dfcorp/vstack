...
- **Rebuild serialization mirrors the CLI's native records** (`convert.ts`): a successful MCP tool result is an array of text blocks, an error result a plain string, `is_error` is present only when true, and the `timeout: 120` that `mapToolArgs` injects into bash calls is stripped again from `tool_use.input`. A rebuild can still diverge where the native transcript holds CLI-generated attachments (`total_tokens_reminder` and friends) that pi history does not carry, so a rebuild after an abort still re-writes the native segment since the previous rebuild; rebuilds are therefore worth avoiding, not just making cheap.

...
- **`systemPrompt.snapshot: true`.** The CLI records the rendered prompt as a `prompt_snapshot` attachment on the first launch and replays it verbatim on every later request and resume, so the appended AGENTS.md/skills text is frozen for the conversation, like CLAUDE.md. `syncSharedSession` carries the last `prompt_snapshot` record into a rebuilt jsonl (`appendPromptSnapshotRecord`) so a rebuild does not re-render the prompt either.
